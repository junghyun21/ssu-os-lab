...
일부분이 전체와 비슷한 형태를 띄고 있으며, 모든 점(공간)을 한 번 이상 지나는 프랙탈 공간 채움 곡선 중 하나이다. 이는 한 변의 길이가 2의 제곱수인 정사각형 그리드의 모든 점을 방문한다.

![img](./img/hilbert.png)

네 개의 컵(cup)과 각 컵을 이어주는 세 개의 조인(join)으로 이루어져 있으며, 각 컵과 조인은 시작 위치 및 진행 방향에 따라 4가지 규칙으로 나눌 수 있다.

![img](./img/hilbert2.png)

이전(n/2)의 컵을 기준으로 `1. 두번째와 네번째의 순서를 바꾼 컵`을 그리고 `2. 기존의 순서 그대로 그린 컵`을 그리고 `3. 기존의 순서 그대로 그린 컵`을 그리고 `3. 첫번째와 세번째의 순서를 바꾼 컵`을 그린다. 이 때 각 컵을 연결하는 조인(join)은 이전 컵의 진행 방향과 동일한 모양을 띈다. 좌측 하단을 시작으로 좌측 상단, 우측 상단, 우측 하단 순서로 컵을 그려나간다고 가정했을 때 숫자를 이용하여 순서를 시각화하면 다음과 같다. 

![img](./img/hilbert3.png)

이를 활용하여 재귀를 통해 아래의 코드를 작성하였으며, 좌측 하단을 (1, 1)로 가정하였다.

- 좌표가 좌측 하단에 있는 경우: (x, y)
- 좌표가 좌측 상단에 있는 경우: (x, y + n/2)
- 좌표가 우측 상단에 있는 경우: (x + n/2, y + n/2)
- 좌표가 우측 하단에 있는 경우: (x + n/2, y)

[source](https://github.com/junghyun21/coding-test/tree/2fad399ae8b2dc799d6c3079e937b78e3574e568/%EB%B0%B1%EC%A4%80/Gold/14956.%E2%80%85Philosopher%E2%80%99s%E2%80%85Walk)
